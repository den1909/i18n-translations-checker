...
function pruneAllUnusedKeys(i18nDir, srcPath, logger = console.log) {
  const projectRoot = process.cwd();
  const usedMap = getUsedTranslationKeys(srcPath, projectRoot);
  ...
  for (const file of files) {
    ...
    for (const key of Object.keys(flat)) {
      if (!usedKeys.has(key)) delete flat[key];
    }
    const after = Object.keys(flat).length;
    const pruned = before - after;
    totalPruned += pruned;
    perFile.push({ file, pruned });
    if (pruned > 0) {
      fs.writeFileSync(filePath, JSON.stringify(unflatten(flat), null, 2));
      logger(`🧹 Pruned ${pruned} unused key(s) in "${file}"`);
    } else {
      logger(`🧹 No unused keys in "${file}"`);
    }
  }

  logger(`\n🧾 Total pruned: ${totalPruned}`);
  return { totalPruned, perFile };
}
